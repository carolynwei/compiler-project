...
public class ASTPrinter implements ASTVisitor<Void> {
    ...
    private void printNode(String nodeType) {
        ...
        output.append(nodeType).append("\n");
    }
    
    private void appendNode(String nodeType, String details) {
        appendIndent();
        output.append(nodeType).append(": ").append(details).append("\n");
    }
    
    private void appendNode(String nodeType) {
        appendIndent();
        output.append(nodeType).append("\n");
    }
    ...
    public Void visitFieldDeclaration(FieldDeclarationNode node) {
        printNode("FieldDeclaration", node.getDeclarator().getName());
        indentLevel++;
    ...
    public Void visitDeclaratorNode(DeclaratorNode node) {
        printNode("Declarator", node.getName());
        return null;
    }
    ...
    @Override
    public Void visitShiftExpressionNode(ShiftExpressionNode node) {
        printNode("ShiftExpression", node.getOperator().toString());
        indentLevel++;
        node.getLeft().accept(this);
        node.getRight().accept(this);
        indentLevel--;
    ...
    @Override
    public Void visitBitwiseExpressionNode(BitwiseExpressionNode node) {
        printNode("BitwiseExpression", node.getOperator().toString());
        indentLevel++;
        node.getLeft().accept(this);
        node.getRight().accept(this);
        indentLevel--;
