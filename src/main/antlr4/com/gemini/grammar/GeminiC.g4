...
parameter: type ID (LBRACKET RBRACKET)*;

// 变量声明
variableDeclaration: type variableDeclarator (COMMA variableDeclarator)* SEMICOLON;

variableDeclarator: ID (LBRACKET expression RBRACKET)* (ASSIGN (expression | arrayInitializer))?;
...
// for 语句
// 修复：forInit 使用不带分号的变量声明或表达式
forStatement: FOR LPAREN forInit? SEMICOLON expression? SEMICOLON forUpdate? RPAREN statement;

forInit: 
    type variableDeclarator (COMMA variableDeclarator)*   // int i = 0, j = 1（不带分号）
    | expression                                          // i = 0（不带分号）
    ;

forUpdate: 
    expression            // i++, i += 1 等（不带分号）
    ;
...
// 类型转换表达式
// 修复：添加 C 风格的类型转换支持 (type) expression
castExpression:
    unaryExpression
    | LPAREN type RPAREN castExpression     // (float)i, (int)x 等
    ;
